...
// Importar los componentes de filtros actualizados
import FromToDate from "./FromToDates"
import { SelectStatus, SelectLineaInvestigacion } from "./SelectedFilter"
...
interface TableFiltersProps {
  ...
  filterOrder?: ("dates" | "status" | "lineaInvestigacion" | "config")[]
  // Opción para especificar qué filtros están siempre activos
  alwaysActiveFilters?: ("dates" | "status" | "lineaInvestigacion" | "config")[]
  alwaysActivePosition?: "before" | "after"
  // Nueva opción para especificar qué filtros nunca se muestran
  alwaysHiddenFilters?: ("dates" | "status" | "lineaInvestigacion" | "config")[]
  // Nueva prop para ocultar el botón de toggle
  hideToggleButton?: boolean
}
...
const TableFilters: React.FC<TableFiltersProps> = ({
  ...
  const renderFilterComponent = (filterType: "dates" | "status" | "lineaInvestigacion" | "config") => {
    switch (filterType) {
      case "status":
        return <SelectStatus />
      case "lineaInvestigacion":
        return <SelectLineaInvestigacion />
      case "dates":
        return <FromToDate />
      case "config":
